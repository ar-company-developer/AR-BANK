...
// --- Нова логіка синхронізації другої карти ---
async function syncSecondCard(localData, uid) {
    ...
    // Отримуємо номер карти без пробілів для пошуку в індексі
    ...
    try {
        ...
        if (indexSnap.exists()) {
            ...
            // ПІДПИСУЄМОСЬ НА ОНОВЛЕННЯ ВЛАСНИКА
            // Це ключовий момент: тепер баланс оновиться автоматично у всіх
            onValue(ref(database, `user/${ownerUID}/card2`), (snap) => {
                if (snap.exists()) {
                    renderSecondCardUI(snap.val());
                }
            });
        } else {
            // Якщо в індексі немає (наприклад, помилка), рендеримо локальні дані
...
document.addEventListener("DOMContentLoaded", () => {
    onAuthStateChanged(auth, user => {
        if (user) {
            ...
            // Використовуємо нову функцію синхронізації
            onValue(ref(database, `user/${user.uid}/card2`), (snap) => {
                syncSecondCard(snap.val(), user.uid);
            }, { onlyOnce: true }); // Слухаємо локальні дані один раз, щоб запустити onValue для власника
